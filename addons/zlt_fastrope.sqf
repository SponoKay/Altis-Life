...
#define STR_TOSS_ROPES "Lancer des cordes"
#define STR_FAST_ROPE "Descente en rappel"
#define STR_CUT_ROPES "Couper cordes"


if (isdedicated) exitwith {};
waituntil {player == player && (playerSide == West)};
...
player addAction["<t color='#AAF200'>"+STR_TOSS_ROPES+"</t>", zlt_fnc_createropes, [], -1, false, false, '','[] call zlt_fnc_ropes_cond'];
player addAction["<t color='#FF9900'>"+STR_CUT_ROPES+"</t>", zlt_fnc_removeropes, [], -1, false, false, '','not zlt_mutexAction and count ((vehicle player) getvariable ["zlt_ropes", []]) != 0'];
player addAction["<t color='#AAF200'>"+STR_FAST_ROPE+"</t>", zlt_fnc_fastrope, [], 15, false, false, '','not zlt_mutexAction and count ((vehicle player) getvariable ["zlt_ropes", []]) != 0 and player != driver vehicle player'];
