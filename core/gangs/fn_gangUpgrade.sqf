...
if(_action) then {
	if(R6cuXKTCWDZFxfbfdATm < _upgradePrice) exitWith {
		hint parseText format[
			...
			[R6cuXKTCWDZFxfbfdATm] call life_fnc_numberText,
			[_upgradePrice - R6cuXKTCWDZFxfbfdATm] call life_fnc_numberText
		];
	};
	__SUB__(R6cuXKTCWDZFxfbfdATm,_upgradePrice);
