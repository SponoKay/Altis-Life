...
if(R6cuXKTCWDZFxfbfdATm < (__GETC__(life_gangPrice))) exitWith {
	hint format[localize "STR_GNOTF_NotEnoughMoney",[((__GETC__(life_gangPrice))-R6cuXKTCWDZFxfbfdATm)] call life_fnc_numberText];
	{(group player) setVariable[_x,nil,true];} foreach ["gang_id","gang_owner","gang_name","gang_members","gang_maxmembers","gang_bank"];
};

__SUB__(R6cuXKTCWDZFxfbfdATm,(__GETC__(life_gangPrice)));
