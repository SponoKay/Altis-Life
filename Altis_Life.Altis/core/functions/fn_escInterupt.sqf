...
_canUseRespawnFnc = {
	if (!life_session_completed) exitWith {false};
...
_canUseAbortFnc = {
	if (playerSide == west) exitWith {true};
	if ((player getVariable["restrained",FALSE]) OR (player getVariable["Escorting",FALSE]) OR (player getVariable["transporting",FALSE]) OR (life_istazed) OR (player getVariable ["FAR_isUnconscious", 0] == 1)) then {false} else {true};
};
...
while {true} do
{
	...
	_canUseAbort = call _canUseRespawnFnc;
	_canUseAbort spawn _escSync;
	
	_canUseRespawn = call _canUseRespawnFnc;
	if (_canUseRespawn) then {
