...
if(_hasLicense) then
{
	...
	if((count _itemInfo == 5) && !([false,_oldItem2,_oldVal2] call life_fnc_handleInv)) exitWith {5 cutText ["","PLAIN"]; life_is_processing = false;};
...
{
	if(hBf0MznDHQm7X5DFFSmy < _cost) exitWith {hint format[localize "STR_Process_License",[_cost] call life_fnc_numberText]; 5 cutText ["","PLAIN"]; life_is_processing = false;};
	...
	hBf0MznDHQm7X5DFFSmy = hBf0MznDHQm7X5DFFSmy - _cost;
	life_is_processing = false;
};	